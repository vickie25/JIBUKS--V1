...
export default function SalesOrderScreen() {
    ...
    const termOptions = [
        { id: 'Due on Receipt', name: 'Due on Receipt', days: 0 },
    ...
    useEffect(() => {
        // Update due date when terms or order date changes
        const selectedTerm = termOptions.find(t => t.id === terms);
        if (selectedTerm) {
    ...
    const loadData = async () => {
        try {
            setLoading(true);
            const [customersData, itemsData, vatRatesData] = await Promise.all([
            ...
            setSalesOrderNo('SO-' + Math.floor(1000 + Math.random() * 9000));
        } catch (error) {
    ...
    const updateLineItem = (index: number, field: keyof LineItem, value: any) => {
        ...
        if (field === 'itemId') {
            ...
            if (item) {
                ...
                updated[index].rate = item.sellingPrice || item.price || '0';
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Assuming a sales-order endpoint exists or using generic invoice endpoint
            ...
            Alert.alert('Error', error.error || 'Failed to save Sales Order');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <View style={styles.container}>
                ...
                <SafeAreaView edges={['top']}>
                    <View style={styles.headerContent}>
                        ...
                        <View style={styles.headerTitleContainer}>
                            ...
                            <Text style={styles.headerSubtitle}>Create Booking Order</Text>
                        </View>
            ...
            >
                ...
                >
                    ...
                    <View style={styles.card}>
                            ...
                            <View style={[styles.inputGroup, { flex: 1.5 }]}>
                                ...
                                >
                                    <Text style={styles.dropdownValue} numberOfLines={1}>
                                        {selectedCustomerId ? customers.find(c => c.id === selectedCustomerId)?.name : 'Search and select'}
                                    </Text>
                                    <Feather name="search" size={18} color={PRIMARY_BLUE} />
                                </TouchableOpacity>
                            </View>
                            <View style={[styles.inputGroup, { flex: 1, marginLeft: 10 }]}>
                                <Text style={styles.inputLabel}>SALES ORDER NO.</Text>
                                <TextInput
                        ...
                        <View style={styles.cardRow}>
                            <View style={[styles.inputGroup, { flex: 1 }]}>
                                <Text style={styles.inputLabel}>BILL TO (ADDRESS)</Text>
                                <TextInput
                        ...
                        <View style={[styles.cardRow, { marginTop: 15 }]}>
                            <View style={[styles.inputGroup, { flex: 1 }]}>
                                ...
                                >
                                    <Text style={styles.dropdownValue}>{terms}</Text>
                                    <Feather name="clock" size={16} color={PRIMARY_BLUE} />
                                </TouchableOpacity>
                            </View>
                            <View style={[styles.inputGroup, { flex: 1, marginLeft: 10 }]}>
                            ...
                            <View style={styles.itemHeader}>
                                ...
                                >
                                    <Text style={styles.itemTitle} numberOfLines={1}>
                                        {item.itemId ? inventoryItems.find(i => String(i.id) === item.itemId)?.name : 'Select Product / Service'}
                                    </Text>
                    ...
                    <View style={styles.totalsCard}>
                        ...
                        <View style={styles.grandTotalRow}>
                            <Text style={styles.grandTotalLabel}>SALES ORDER TOTAL</Text>
                    ...
                    <View style={styles.card}>
                        <View style={styles.inputGroup}>
                            ...
                            <TextInput
                                ...
                                onChangeText={setNoteToCustomer}
                                placeholder="Thank you for your business!"
                            />
                        </View>
                        <View style={[styles.inputGroup, { marginTop: 15 }]}>
                            <Text style={styles.inputLabel}>INTERNAL MEMO (HIDDEN)</Text>
                            <TextInput
                                style={[styles.textInput, styles.multilineInput]}
                                multiline
                                value={internalMemo}
                    ...
                    <View style={styles.actionRow}>
                        <TouchableOpacity style={styles.clearBtn} onPress={() => {
                            ...
                            setLineItems([{ id: '1', itemId: null, description: '', quantity: '1', rate: '', amount: '0', taxTreatment: 'Exclusive of Tax', vatRateId: null }]);
                            setNoteToCustomer('');
                            setInternalMemo('');
                            setContactInfo('');
                        }}>
                            <Feather name="refresh-ccw" size={18} color={TEXT_MUTED} />
                    ...
                    <TouchableOpacity
                        ...
                        disabled={loading}
                    >
                        <Text style={styles.saveAndNewText}>Save and Create New</Text>
                        <Ionicons name="arrow-forward-circle" size={20} color={PRIMARY_BLUE} />
                    </TouchableOpacity>
            ...
            <SelectionModal
                ...
                title="Payment Terms"
                data={termOptions}
                onSelect={(id: string | number) => { setTerms(String(id)); setShowTermsModal(false); }}
            ...
            <DatePickerModal
                ...
                date={orderDate}
                onSelect={(d: Date) => { setOrderDate(d); setShowOrderDatePicker(false); }}
...
// Reusable Components (Matching Purchase Order for Consistency)
...
const styles = StyleSheet.create({
    ...
    clearBtn: { flex: 1, height: 60, borderRadius: 18, borderWidth: 1.5, borderColor: BORDER_COLOR, alignItems: 'center', justifyContent: 'center', backgroundColor: '#fff', flexDirection: 'row', gap: 8 },
    clearBtnText: { color: TEXT_MUTED, fontSize: 13, fontWeight: 'bold' },
    saveBtn: { flex: 2, borderRadius: 18, overflow: 'hidden', elevation: 8, shadowColor: PRIMARY_BLUE, shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.3, shadowRadius: 15 },
    saveBtnGradient: { height: 60, justifyContent: 'center', alignItems: 'center' },
    saveBtnText: { color: '#fff', fontSize: 17, fontWeight: 'bold', letterSpacing: 0.5 },
    saveAndNewBtn: { height: 50, marginTop: 15, borderRadius: 15, backgroundColor: LIGHT_BLUE, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, borderStyle: 'dashed', borderWidth: 1, borderColor: PRIMARY_BLUE },
    saveAndNewText: { color: PRIMARY_BLUE, fontWeight: 'bold', fontSize: 14 },
